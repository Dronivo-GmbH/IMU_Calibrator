...
def relative_rotation_from_level(
    ax: float, ay: float, az: float,
    ax0: float, ay0: float, az0: float,
) -> np.ndarray:
    """Rotation from current body frame to the captured level frame."""
    r0 = rotation_from_accel(ax0, ay0, az0)
    r1 = rotation_from_accel(ax, ay, az)
    return r0.T @ r1


def draw_orientation_block_relative(
    ax: Axes,
    rotation: np.ndarray,
    *,
    elev: float = 28.0,
    ...
    """Draw IMU block tilted relative to the captured level pose."""
    ax.cla()
    ax.set_facecolor(theme.PANEL)

    hx, hy, hz = 0.85, 0.62, 0.14
    corners = _cube_corners(hx, hy, hz) @ rotation.T
    ...
    for label, col, idx in (
        ...
        direction = rotation[:, idx] * arrow_len
        ax.quiver(
    ...
    if roll_deg is not None and pitch_deg is not None:
        title = f"vs level  ·  Roll {roll_deg:+.1f}°  ·  Pitch {pitch_deg:+.1f}°"
    else:
        title = "Orientation vs level"
